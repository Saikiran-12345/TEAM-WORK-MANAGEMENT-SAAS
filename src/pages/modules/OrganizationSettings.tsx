...
// OrganizationSettings Data Model
...
export const OrganizationSettingsView: React.FC = () => {
  const [data, setData] = useState<OrganizationSettingsRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('OrganizationSettings_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: OrganizationSettingsRecord = {
    ...
    setData(newData);
    localStorage.setItem('OrganizationSettings_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: OrganizationSettingsRecord) => {
