...
export const InvoicesView: React.FC = () => {
  const [data, setData] = useState<InvoicesRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('Invoices_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: InvoicesRecord = {
    ...
    setData(newData);
    localStorage.setItem('Invoices_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: InvoicesRecord) => {
