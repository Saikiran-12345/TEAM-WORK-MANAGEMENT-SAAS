...
// ExpenseReports Data Model
export interface ExpenseReportsRecord {
  id: string;
  field1: string;
  field2: string;
  field3: string;
...
export const ExpenseReportsView: React.FC = () => {
  const [data, setData] = useState<ExpenseReportsRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('ExpenseReports_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: ExpenseReportsRecord = {
    ...
    setData(newData);
    localStorage.setItem('ExpenseReports_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: ExpenseReportsRecord) => {
