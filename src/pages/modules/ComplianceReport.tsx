...
export const ComplianceReportView: React.FC = () => {
  const [data, setData] = useState<ComplianceReportRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('ComplianceReport_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: ComplianceReportRecord = {
    ...
    setData(newData);
    localStorage.setItem('ComplianceReport_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: ComplianceReportRecord) => {
