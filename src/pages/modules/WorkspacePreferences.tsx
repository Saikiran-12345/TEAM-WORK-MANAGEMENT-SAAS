...
// WorkspacePreferences Data Model
...
export const WorkspacePreferencesView: React.FC = () => {
  const [data, setData] = useState<WorkspacePreferencesRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('WorkspacePreferences_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: WorkspacePreferencesRecord = {
    ...
    setData(newData);
    localStorage.setItem('WorkspacePreferences_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: WorkspacePreferencesRecord) => {
