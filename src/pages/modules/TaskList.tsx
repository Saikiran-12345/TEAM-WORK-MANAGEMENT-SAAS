...
export const TaskListView: React.FC = () => {
  const [data, setData] = useState<TaskListRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('TaskList_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: TaskListRecord = {
    ...
    setData(newData);
    localStorage.setItem('TaskList_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: TaskListRecord) => {
