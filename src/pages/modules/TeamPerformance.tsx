...
export const TeamPerformanceView: React.FC = () => {
  const [data, setData] = useState<TeamPerformanceRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('TeamPerformance_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: TeamPerformanceRecord = {
    ...
    setData(newData);
    localStorage.setItem('TeamPerformance_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: TeamPerformanceRecord) => {
