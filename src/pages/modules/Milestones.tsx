...
export const MilestonesView: React.FC = () => {
  const [data, setData] = useState<MilestonesRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('Milestones_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: MilestonesRecord = {
    ...
    setData(newData);
    localStorage.setItem('Milestones_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: MilestonesRecord) => {
