...
export const RiskManagementView: React.FC = () => {
  const [data, setData] = useState<RiskManagementRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('RiskManagement_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: RiskManagementRecord = {
    ...
    setData(newData);
    localStorage.setItem('RiskManagement_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: RiskManagementRecord) => {
