...
export const JiraImportView: React.FC = () => {
  const [data, setData] = useState<JiraImportRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('JiraImport_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: JiraImportRecord = {
    ...
    setData(newData);
    localStorage.setItem('JiraImport_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: JiraImportRecord) => {
