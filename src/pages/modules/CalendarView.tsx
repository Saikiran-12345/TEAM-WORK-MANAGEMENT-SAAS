...
export const CalendarViewView: React.FC = () => {
  const [data, setData] = useState<CalendarViewRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('CalendarView_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: CalendarViewRecord = {
    ...
    setData(newData);
    localStorage.setItem('CalendarView_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: CalendarViewRecord) => {
