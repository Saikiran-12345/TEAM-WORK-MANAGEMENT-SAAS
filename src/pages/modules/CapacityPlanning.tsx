...
export const CapacityPlanningView: React.FC = () => {
  const [data, setData] = useState<CapacityPlanningRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('CapacityPlanning_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: CapacityPlanningRecord = {
    ...
    setData(newData);
    localStorage.setItem('CapacityPlanning_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: CapacityPlanningRecord) => {
