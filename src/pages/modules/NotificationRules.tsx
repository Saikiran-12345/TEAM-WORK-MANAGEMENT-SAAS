...
// NotificationRules Data Model
export interface NotificationRulesRecord {
  id: string;
  field1: string;
  field2: string;
  field3: string;
...
export const NotificationRulesView: React.FC = () => {
  const [data, setData] = useState<NotificationRulesRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('NotificationRules_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: NotificationRulesRecord = {
    ...
    setData(newData);
    localStorage.setItem('NotificationRules_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: NotificationRulesRecord) => {
