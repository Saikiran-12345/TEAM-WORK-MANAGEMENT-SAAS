...
export const ProjectDetailsView: React.FC = () => {
  const [data, setData] = useState<ProjectDetailsRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('ProjectDetails_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: ProjectDetailsRecord = {
    ...
    setData(newData);
    localStorage.setItem('ProjectDetails_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: ProjectDetailsRecord) => {
