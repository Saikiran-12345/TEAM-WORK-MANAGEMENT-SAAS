...
export const ExportDataView: React.FC = () => {
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('ExportData_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: ExportDataRecord = {
    ...
    setData(newData);
    localStorage.setItem('ExportData_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: ExportDataRecord) => {
