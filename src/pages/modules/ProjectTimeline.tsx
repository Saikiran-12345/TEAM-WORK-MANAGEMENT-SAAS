...
export const ProjectTimelineView: React.FC = () => {
  const [data, setData] = useState<ProjectTimelineRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('ProjectTimeline_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: ProjectTimelineRecord = {
    ...
    setData(newData);
    localStorage.setItem('ProjectTimeline_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: ProjectTimelineRecord) => {
