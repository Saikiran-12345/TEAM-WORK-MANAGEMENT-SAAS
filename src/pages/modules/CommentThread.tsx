...
export const CommentThreadView: React.FC = () => {
  const [data, setData] = useState<CommentThreadRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('CommentThread_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: CommentThreadRecord = {
    ...
    setData(newData);
    localStorage.setItem('CommentThread_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: CommentThreadRecord) => {
