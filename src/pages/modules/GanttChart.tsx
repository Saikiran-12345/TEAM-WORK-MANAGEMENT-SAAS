...
// GanttChart Data Model
export interface GanttChartRecord {
  id: string;
  field1: string;
  field2: string;
  field3: string;
...
export const GanttChartView: React.FC = () => {
  const [data, setData] = useState<GanttChartRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('GanttChart_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: GanttChartRecord = {
    ...
    setData(newData);
    localStorage.setItem('GanttChart_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: GanttChartRecord) => {
