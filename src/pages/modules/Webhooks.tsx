...
export const WebhooksView: React.FC = () => {
  const [data, setData] = useState<WebhooksRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('Webhooks_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: WebhooksRecord = {
    ...
    setData(newData);
    localStorage.setItem('Webhooks_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: WebhooksRecord) => {
