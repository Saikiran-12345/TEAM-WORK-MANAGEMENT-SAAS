...
export const BudgetTrackingView: React.FC = () => {
  const [data, setData] = useState<BudgetTrackingRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('BudgetTracking_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: BudgetTrackingRecord = {
    ...
    setData(newData);
    localStorage.setItem('BudgetTracking_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: BudgetTrackingRecord) => {
