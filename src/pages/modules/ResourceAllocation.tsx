...
export const ResourceAllocationView: React.FC = () => {
  const [data, setData] = useState<ResourceAllocationRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('ResourceAllocation_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: ResourceAllocationRecord = {
    ...
    setData(newData);
    localStorage.setItem('ResourceAllocation_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: ResourceAllocationRecord) => {
