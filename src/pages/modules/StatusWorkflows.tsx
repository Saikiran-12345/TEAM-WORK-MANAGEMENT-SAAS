...
// StatusWorkflows Data Model
export interface StatusWorkflowsRecord {
  id: string;
  field1: string;
  field2: string;
  field3: string;
...
export const StatusWorkflowsView: React.FC = () => {
  const [data, setData] = useState<StatusWorkflowsRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('StatusWorkflows_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: StatusWorkflowsRecord = {
    ...
    setData(newData);
    localStorage.setItem('StatusWorkflows_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: StatusWorkflowsRecord) => {
