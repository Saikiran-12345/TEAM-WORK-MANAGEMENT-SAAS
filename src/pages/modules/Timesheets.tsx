...
export const TimesheetsView: React.FC = () => {
  const [data, setData] = useState<TimesheetsRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('Timesheets_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: TimesheetsRecord = {
    ...
    setData(newData);
    localStorage.setItem('Timesheets_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: TimesheetsRecord) => {
