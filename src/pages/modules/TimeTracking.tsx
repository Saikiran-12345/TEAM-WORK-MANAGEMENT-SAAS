...
export const TimeTrackingView: React.FC = () => {
  const [data, setData] = useState<TimeTrackingRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('TimeTracking_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: TimeTrackingRecord = {
    ...
    setData(newData);
    localStorage.setItem('TimeTracking_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: TimeTrackingRecord) => {
