...
// LeaveManagement Data Model
export interface LeaveManagementRecord {
  id: string;
  field1: string;
  field2: string;
  field3: string;
...
export const LeaveManagementView: React.FC = () => {
  const [data, setData] = useState<LeaveManagementRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('LeaveManagement_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: LeaveManagementRecord = {
    ...
    setData(newData);
    localStorage.setItem('LeaveManagement_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: LeaveManagementRecord) => {
