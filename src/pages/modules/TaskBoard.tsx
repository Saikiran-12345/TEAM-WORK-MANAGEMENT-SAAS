...
export const TaskBoardView: React.FC = () => {
  const [data, setData] = useState<TaskBoardRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('TaskBoard_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: TaskBoardRecord = {
    ...
    setData(newData);
    localStorage.setItem('TaskBoard_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: TaskBoardRecord) => {
