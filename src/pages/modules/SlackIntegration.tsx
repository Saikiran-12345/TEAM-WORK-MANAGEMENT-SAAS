...
export const SlackIntegrationView: React.FC = () => {
  const [data, setData] = useState<SlackIntegrationRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('SlackIntegration_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: SlackIntegrationRecord = {
    ...
    setData(newData);
    localStorage.setItem('SlackIntegration_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: SlackIntegrationRecord) => {
