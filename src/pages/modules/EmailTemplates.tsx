...
// EmailTemplates Data Model
export interface EmailTemplatesRecord {
  id: string;
  field1: string;
  field2: string;
  field3: string;
...
export const EmailTemplatesView: React.FC = () => {
  const [data, setData] = useState<EmailTemplatesRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('EmailTemplates_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: EmailTemplatesRecord = {
    ...
    setData(newData);
    localStorage.setItem('EmailTemplates_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: EmailTemplatesRecord) => {
