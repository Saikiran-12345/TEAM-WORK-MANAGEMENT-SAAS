...
export const GithubIntegrationView: React.FC = () => {
  const [data, setData] = useState<GithubIntegrationRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('GithubIntegration_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: GithubIntegrationRecord = {
    ...
    setData(newData);
    localStorage.setItem('GithubIntegration_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: GithubIntegrationRecord) => {
