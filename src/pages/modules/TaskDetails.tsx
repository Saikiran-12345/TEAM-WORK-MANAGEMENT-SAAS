...
export const TaskDetailsView: React.FC = () => {
  const [data, setData] = useState<TaskDetailsRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('TaskDetails_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: TaskDetailsRecord = {
    ...
    setData(newData);
    localStorage.setItem('TaskDetails_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: TaskDetailsRecord) => {
