...
export const RolePermissionsView: React.FC = () => {
  const [data, setData] = useState<RolePermissionsRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('RolePermissions_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: RolePermissionsRecord = {
    ...
    setData(newData);
    localStorage.setItem('RolePermissions_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: RolePermissionsRecord) => {
