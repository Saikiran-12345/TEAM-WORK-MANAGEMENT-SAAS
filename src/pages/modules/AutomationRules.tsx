...
// AutomationRules Data Model
export interface AutomationRulesRecord {
  id: string;
  field1: string;
  field2: string;
  field3: string;
...
export const AutomationRulesView: React.FC = () => {
  const [data, setData] = useState<AutomationRulesRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('AutomationRules_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: AutomationRulesRecord = {
    ...
    setData(newData);
    localStorage.setItem('AutomationRules_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: AutomationRulesRecord) => {
