...
// CustomFields Data Model
export interface CustomFieldsRecord {
  id: string;
  field1: string;
  field2: string;
  field3: string;
...
export const CustomFieldsView: React.FC = () => {
  const [data, setData] = useState<CustomFieldsRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('CustomFields_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: CustomFieldsRecord = {
    ...
    setData(newData);
    localStorage.setItem('CustomFields_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: CustomFieldsRecord) => {
