...
export const ClientPortalView: React.FC = () => {
  const [data, setData] = useState<ClientPortalRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('ClientPortal_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: ClientPortalRecord = {
    ...
    setData(newData);
    localStorage.setItem('ClientPortal_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: ClientPortalRecord) => {
