...
export const SecurityAuditView: React.FC = () => {
  const [data, setData] = useState<SecurityAuditRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('SecurityAudit_data');
  ...
  const handleSave = () => {
    ...
    setData(newData);
    localStorage.setItem('SecurityAudit_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: SecurityAuditRecord) => {
