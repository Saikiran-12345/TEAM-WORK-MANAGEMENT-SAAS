...
export const ActivityLogsView: React.FC = () => {
  const [data, setData] = useState<ActivityLogsRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('ActivityLogs_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: ActivityLogsRecord = {
    ...
    setData(newData);
    localStorage.setItem('ActivityLogs_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: ActivityLogsRecord) => {
