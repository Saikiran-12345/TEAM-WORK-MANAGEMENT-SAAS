...
export const ThemeSettingsView: React.FC = () => {
  const [data, setData] = useState<ThemeSettingsRecord[]>([]);
  ...
  const loadData = () => {
    ...
    setTimeout(() => {
      const raw = localStorage.getItem('ThemeSettings_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: ThemeSettingsRecord = {
    ...
    setData(newData);
    localStorage.setItem('ThemeSettings_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: ThemeSettingsRecord) => {
