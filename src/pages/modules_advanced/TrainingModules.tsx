...
// TrainingModules Advanced Data Model
export interface TrainingModulesAdvancedRecord {
  id: string;
...
export const TrainingModulesAdvancedView: React.FC = () => {
  const [data, setData] = useState<TrainingModulesAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_TrainingModules_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: TrainingModulesAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_TrainingModules_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: TrainingModulesAdvancedRecord) => {
