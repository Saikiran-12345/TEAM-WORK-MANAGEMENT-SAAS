...
// ApiUsage Advanced Data Model
export interface ApiUsageAdvancedRecord {
  id: string;
...
export const ApiUsageAdvancedView: React.FC = () => {
  const [data, setData] = useState<ApiUsageAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_ApiUsage_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: ApiUsageAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_ApiUsage_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: ApiUsageAdvancedRecord) => {
