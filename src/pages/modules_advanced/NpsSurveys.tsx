...
// NpsSurveys Advanced Data Model
export interface NpsSurveysAdvancedRecord {
  id: string;
...
export const NpsSurveysAdvancedView: React.FC = () => {
  const [data, setData] = useState<NpsSurveysAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_NpsSurveys_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: NpsSurveysAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_NpsSurveys_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: NpsSurveysAdvancedRecord) => {
