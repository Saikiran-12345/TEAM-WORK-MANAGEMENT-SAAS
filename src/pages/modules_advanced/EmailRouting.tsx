...
// EmailRouting Advanced Data Model
export interface EmailRoutingAdvancedRecord {
  id: string;
...
export const EmailRoutingAdvancedView: React.FC = () => {
  const [data, setData] = useState<EmailRoutingAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_EmailRouting_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: EmailRoutingAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_EmailRouting_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: EmailRoutingAdvancedRecord) => {
