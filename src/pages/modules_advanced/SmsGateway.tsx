...
// SmsGateway Advanced Data Model
export interface SmsGatewayAdvancedRecord {
  id: string;
...
export const SmsGatewayAdvancedView: React.FC = () => {
  const [data, setData] = useState<SmsGatewayAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_SmsGateway_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: SmsGatewayAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_SmsGateway_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: SmsGatewayAdvancedRecord) => {
