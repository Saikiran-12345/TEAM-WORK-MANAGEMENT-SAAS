...
// Localization Advanced Data Model
export interface LocalizationAdvancedRecord {
  id: string;
...
export const LocalizationAdvancedView: React.FC = () => {
  const [data, setData] = useState<LocalizationAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_Localization_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: LocalizationAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_Localization_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: LocalizationAdvancedRecord) => {
