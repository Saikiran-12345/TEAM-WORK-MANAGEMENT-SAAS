...
// WebhookDelivery Advanced Data Model
export interface WebhookDeliveryAdvancedRecord {
  id: string;
...
export const WebhookDeliveryAdvancedView: React.FC = () => {
  const [data, setData] = useState<WebhookDeliveryAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_WebhookDelivery_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: WebhookDeliveryAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_WebhookDelivery_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: WebhookDeliveryAdvancedRecord) => {
