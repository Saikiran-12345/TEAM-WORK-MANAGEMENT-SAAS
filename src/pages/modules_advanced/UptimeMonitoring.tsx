...
// UptimeMonitoring Advanced Data Model
export interface UptimeMonitoringAdvancedRecord {
  id: string;
...
export const UptimeMonitoringAdvancedView: React.FC = () => {
  const [data, setData] = useState<UptimeMonitoringAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_UptimeMonitoring_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: UptimeMonitoringAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_UptimeMonitoring_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: UptimeMonitoringAdvancedRecord) => {
