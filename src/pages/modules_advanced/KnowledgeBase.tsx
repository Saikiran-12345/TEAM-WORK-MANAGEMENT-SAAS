...
// KnowledgeBase Advanced Data Model
export interface KnowledgeBaseAdvancedRecord {
  id: string;
...
export const KnowledgeBaseAdvancedView: React.FC = () => {
  const [data, setData] = useState<KnowledgeBaseAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_KnowledgeBase_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: KnowledgeBaseAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_KnowledgeBase_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: KnowledgeBaseAdvancedRecord) => {
