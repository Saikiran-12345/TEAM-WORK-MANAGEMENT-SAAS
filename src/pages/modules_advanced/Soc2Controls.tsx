...
// Soc2Controls Advanced Data Model
export interface Soc2ControlsAdvancedRecord {
  id: string;
...
export const Soc2ControlsAdvancedView: React.FC = () => {
  const [data, setData] = useState<Soc2ControlsAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_Soc2Controls_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: Soc2ControlsAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_Soc2Controls_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: Soc2ControlsAdvancedRecord) => {
