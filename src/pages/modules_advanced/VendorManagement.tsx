...
// VendorManagement Advanced Data Model
export interface VendorManagementAdvancedRecord {
  id: string;
...
export const VendorManagementAdvancedView: React.FC = () => {
  const [data, setData] = useState<VendorManagementAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_VendorManagement_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: VendorManagementAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_VendorManagement_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: VendorManagementAdvancedRecord) => {
