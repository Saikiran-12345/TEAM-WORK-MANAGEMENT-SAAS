...
// IncidentResponse Advanced Data Model
export interface IncidentResponseAdvancedRecord {
  id: string;
...
export const IncidentResponseAdvancedView: React.FC = () => {
  const [data, setData] = useState<IncidentResponseAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_IncidentResponse_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: IncidentResponseAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_IncidentResponse_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: IncidentResponseAdvancedRecord) => {
