...
// PushNotifications Advanced Data Model
export interface PushNotificationsAdvancedRecord {
  id: string;
...
export const PushNotificationsAdvancedView: React.FC = () => {
  const [data, setData] = useState<PushNotificationsAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_PushNotifications_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: PushNotificationsAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_PushNotifications_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: PushNotificationsAdvancedRecord) => {
