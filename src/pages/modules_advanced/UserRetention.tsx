...
// UserRetention Advanced Data Model
export interface UserRetentionAdvancedRecord {
  id: string;
...
export const UserRetentionAdvancedView: React.FC = () => {
  const [data, setData] = useState<UserRetentionAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_UserRetention_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: UserRetentionAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_UserRetention_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: UserRetentionAdvancedRecord) => {
