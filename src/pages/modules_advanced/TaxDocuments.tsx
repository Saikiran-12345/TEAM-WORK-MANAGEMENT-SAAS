...
// TaxDocuments Advanced Data Model
export interface TaxDocumentsAdvancedRecord {
  id: string;
...
export const TaxDocumentsAdvancedView: React.FC = () => {
  const [data, setData] = useState<TaxDocumentsAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_TaxDocuments_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: TaxDocumentsAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_TaxDocuments_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: TaxDocumentsAdvancedRecord) => {
