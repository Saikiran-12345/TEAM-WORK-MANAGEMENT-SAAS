...
// DeviceTracking Advanced Data Model
export interface DeviceTrackingAdvancedRecord {
  id: string;
...
export const DeviceTrackingAdvancedView: React.FC = () => {
  const [data, setData] = useState<DeviceTrackingAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_DeviceTracking_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: DeviceTrackingAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_DeviceTracking_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: DeviceTrackingAdvancedRecord) => {
