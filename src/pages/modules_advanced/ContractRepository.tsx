...
// ContractRepository Advanced Data Model
export interface ContractRepositoryAdvancedRecord {
  id: string;
...
export const ContractRepositoryAdvancedView: React.FC = () => {
  const [data, setData] = useState<ContractRepositoryAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_ContractRepository_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: ContractRepositoryAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_ContractRepository_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: ContractRepositoryAdvancedRecord) => {
