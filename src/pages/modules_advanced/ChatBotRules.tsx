...
// ChatBotRules Advanced Data Model
export interface ChatBotRulesAdvancedRecord {
  id: string;
...
export const ChatBotRulesAdvancedView: React.FC = () => {
  const [data, setData] = useState<ChatBotRulesAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_ChatBotRules_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: ChatBotRulesAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_ChatBotRules_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: ChatBotRulesAdvancedRecord) => {
