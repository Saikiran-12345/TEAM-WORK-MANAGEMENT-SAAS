...
// AssetManagement Advanced Data Model
export interface AssetManagementAdvancedRecord {
  id: string;
...
export const AssetManagementAdvancedView: React.FC = () => {
  const [data, setData] = useState<AssetManagementAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_AssetManagement_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: AssetManagementAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_AssetManagement_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: AssetManagementAdvancedRecord) => {
