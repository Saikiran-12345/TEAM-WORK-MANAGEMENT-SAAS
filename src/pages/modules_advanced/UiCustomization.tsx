...
// UiCustomization Advanced Data Model
export interface UiCustomizationAdvancedRecord {
  id: string;
...
export const UiCustomizationAdvancedView: React.FC = () => {
  const [data, setData] = useState<UiCustomizationAdvancedRecord[]>([]);
  ...
  const loadData = () => {
    setIsLoading(true);
    setTimeout(() => {
      const raw = localStorage.getItem('advanced_UiCustomization_data');
  ...
  const handleSave = () => {
    ...
    const newRecord: UiCustomizationAdvancedRecord = {
    ...
    setData(newData);
    localStorage.setItem('advanced_UiCustomization_data', JSON.stringify(newData));
    resetForm();
  };
  ...
  const handleEdit = (record: UiCustomizationAdvancedRecord) => {
